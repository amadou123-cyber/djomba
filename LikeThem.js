...
const LikeThem = () => {
  ...
  const [currentPage, setCurrentPage] = useState(1);
   ...
   const url = `https://meubious.com/api/melike/?page=${currentPage}`;
   const loaddata = () =>
   {
      setLoading(true);
     fetch(url,{
...
return (
  <View >
     ...
     <FlatList 
           ...
           ListHeaderComponent={()=>
            <Text style={{borderWidth:1,borderColor:'green',borderRadius:10,fontStyle:'italic' ,margin:5,padding:5,alignSelf:'center'}}>Les personnes que vous avez aimées seront ici.Si elles vous aiment en retour, vous pouvez chatter avec elles....</Text>}
           ListEmptyComponent={() =>
           (
            !loading ?
            <View style={{ flex:1,justifyContent:'center',alignItems:'center',marginTop:'50%'}}>
              <Text style={{padding:10,margin:10}}>
                Vous avez aimé aucun profil.
              </Text>
              <Icon name='laugh' type='font-awesome-5' size={50} color='grey'></Icon>
            </View>:null
           )}
     data = {datings}
     numColumns={2}
