...
import { View, Share, StyleSheet, SafeAreaView,  Image, StatusBar, FlatList, Dimensions, ScrollView, TouchableOpacity, TextInput, Modal, Alert, ActivityIndicator } from 'react-native';
import { Text, Card, Button, CheckBox, Icon, Dialog } from '@rneui/themed';
...
export default ProfileDetailScreen = () => {
  ...
  console.log(route)
  const {userprofile,setIsLoggedIn,token} = useLogin();
 ...
 const [datas,setDatas] = useState([])
  ...
  const loaddata = () =>
  {
    ...
    .then((json) => {console.log(json.data);setrequest(json.req);setrequest1(json.req1);setDating(json.data); 
...
if (!loading)
  ...
  {userprofile.id !== route.id ?
      (  hasmatch ? 
        <Button onPress={() => { let url = `https://meubious.com/api/create-dialog-dating/${dating.profile}/`
        fetch(url,{
            method: 'POST',
            headers: {
              'Accept': 'application/json',
           ...
           .catch((error) =>console.log(error))}}><Icon name='chatbox-ellipses' type='ionicon' color='white'></Icon> Message</Button>:
        (request ? 
          <>
          ...
          { let url = `https://meubious.com/api/refuse-match/${dating.profile}/`
            fetch(url,{
               ...
               .catch((error) =>console.log(error))}}> Refuser <Icon type='material-community' name='heart-minus' color='white'> </Icon></Button><Button title='Accepter' color='success'onPress={()=> 
                { let url = `https://meubious.com/api/make-match/${dating.profile}/`
                  fetch(url,{
                     ...
                     .catch((error) =>console.log(error))}}> Accepter <Icon type='material-community' name='heart-plus' color='white'></Icon></Button>
          </>: 
          <CheckBox
            containerStyle={{
        ...
        <View style={{ paddingTop: 10, flexDirection: 'row', justifyContent: 'space-between' }}>
          ...
          <View>
          <Text > {userprofile.id !== route.id ?  ( dating.online ? <Ionicons name="ellipse" size={12} color="orange"  > En ligne</Ionicons>: <Ionicons name="ellipse" size={12} color="gray"  > Hors ligne</Ionicons>) :<Ionicons name="ellipse" size={12} color="orange"  > En ligne</Ionicons>}</Text>
